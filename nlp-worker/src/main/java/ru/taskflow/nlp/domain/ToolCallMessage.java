...
/**
 * Одно сообщение в истории диалога с моделью. Роли — как у OpenAI-совместимого
 * API: system, user, assistant, tool.
 * <p>
 * Сообщение ассистента с вызовами инструментов (assistantToolCalls) не влезает
 * в четыре поля записи напрямую — сериализатор ждёт для него отдельный массив
 * tool_calls, а не текст content. Чтобы не раздувать контракт пятым полем,
 * список вызовов упаковывается в content под маркером, который распознаёт
 * только эта запись; наружу (в HTTP-запрос) он никогда не уходит —
 * OpenAiCompatibleToolCallProvider разворачивает его обратно перед сборкой
 * тела запроса.
