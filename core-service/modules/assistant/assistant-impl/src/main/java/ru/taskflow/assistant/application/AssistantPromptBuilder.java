...
public class AssistantPromptBuilder {
    ...
    private static final String SYSTEM_TEMPLATE = """
            ...
            Группы пользователя — единственно допустимые значения параметра group у create_tasks. \
            ...
            2. Перед вызовом create_tasks сверься со списком выше: если там уже есть задача с тем \
            же смыслом, вторую не создавай — работай с найденной через её ярлык (закрой, перенеси \
            срок или измени поля — смотря что сказал пользователь). Это касается каждой задачи из \
            реплики по отдельности, если их несколько.
            ...
            нет такой активной задачи, вызывай create_tasks с этой фразой как title одного из \
            элементов списка tasks. Не требуй слов «создай», «добавь» или «поставь задачу». \
            Прошедшее время — не это: «сходил в кино», «позвонил юристу», «купил корм» — сообщение \
            о сделанном, а не название новой задачи, и это правило на такую фразу не \
            распространяется. Если в списке есть подходящая активная задача — закрывай её \
            (complete_task), а не создавай вторую. Подходящей нет — разбирай реплику как обычную, \
            без применения этого правила: сама по себе фраза в прошедшем времени не создаёт новую \
            задачу.
            4. Заполняй description у элемента tasks, только если в реплике есть подробности сверх \
            ...
            complete_task с task_ref=T1 для первого прочтения, вызови create_tasks с title по смыслу \
            ...
            действует: create_tasks здесь — не дубль, а альтернатива, которую отклонят, если она не \
    ...
    private static final String QUICK_ADD_NOTE =
            ...
            "ассистентом. Если вызываешь mark_ambiguous, ставь create_tasks первым среди действий " +
